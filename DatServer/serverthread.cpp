...
int get_bg_img(std::vector<uchar> & data, unsigned char layer, unsigned char scale,
            ...
            case ID_REQUIRE_BG_IMG:
                {
                    ...
                    if  (packet->length != sizeof(ReqBkImgPkt))
                        ...
                        get_bg_img(data, req_pkt->layer, req_pkt->scale,
                        ...
                        if (!data.empty()) {
                            ...
                            qInfo("Send bg_img l=%d, s=%d, x=%d, y=%d, pkt_size=%d.", req_pkt->layer, req_pkt->scale,
                                  req_pkt->x, req_pkt->y, data.size());
                            rak_peer->Send((char*) &data[0], (int)data.size(), static_cast<PacketPriority> (req_pkt->priority),
                                           RELIABLE, 0, packet->systemAddress, false);
                        }
                    }
                }
                break;
