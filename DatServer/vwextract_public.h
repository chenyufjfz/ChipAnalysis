...
protected:
	...
	*/
	void find_best_bright(const Mat & lg, int d, vector<Point> & loc, bool multi_thread);
	/*
	input ig, raw image
	...
	*/
	int compute_feature(const Mat & img, const Mat & lg, const Mat & grad, int d0, const vector<Point> & loc, Mat features[], bool multi_thread);
	/*
	input img,
	...
	*/
	int feature_extract(const Mat & img, Point & org, Point & range, const vector<int> & d0, Mat & feature, vector<Point> * vs, bool multi_thread);
public:
	ViaML();
...
void convert_element_obj(const vector<ElementObj> & es, vector<MarkObj> & ms);
