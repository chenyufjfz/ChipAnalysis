...
void ConnectView::adjust_viewrect()
{
...
void ConnectView::paintEvent(QPaintEvent *)
{
    if (!render_rect.contains(view_rect) || render_bk_layer !=bk_layer) {
        ...
        render_bkimg(bk_layer, view_rect, size(), RETURN_WHEN_PART_READY, this, true);
        return;
    }
...
void ConnectView::keyPressEvent(QKeyEvent *e)
{
    ...
    case Qt::Key_PageUp:
        if (scale<max_scale) {
            QPoint ce(size().width()*scale, size().height()*scale);
            QSize s(size().width()*scale*2, size().height()*scale*2);
    ...
    if (!render_rect.contains(view_rect))
        render_bkimg(bk_layer, view_rect, size(), RETURN_WHEN_PART_READY, this, true);
    else
        update();
                                    ...
                                    QImage & image, bool finish, const QObject * view)
{
    ...
    if ( !rect.contains(view_rect) || layer!=bk_layer || screen !=size()) {
        ...
        qDebug("render image l=%d,vr=(%d,%d,%d,%d)", layer, render_rect.left(), render_rect.top(),
