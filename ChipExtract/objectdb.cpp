...
ObjectDB::ObjectDB()
{
	for (int i = 0; i < sizeof(layer_wire_info) / sizeof(layer_wire_info[0]); i++)
		layer_wire_info[i] = NULL;
	for (int i = 0; i < sizeof(layer_via_info) / sizeof(layer_via_info[0]); i++)
...
void ObjectDB::get_objects(unsigned char t1, unsigned long long t2_mask, const QRect &r, vector<ElementObj *> & rst)
{
	...
	case OBJ_LINE:
		if (t2_mask & (LINE_NORMAL_WIRE0_MASK | LINE_NORMAL_WIRE1_MASK | LINE_NORMAL_WIRE2_MASK | LINE_NORMAL_WIRE3_MASK))
	...
	case OBJ_POINT:
		...
		if (t2_mask & (POINT_NORMAL_VIA0_MASK | POINT_NORMAL_VIA1_MASK | POINT_NORMAL_VIA2_MASK | POINT_NORMAL_VIA3_MASK))
...
void ObjectDB::add_object(ElementObj & obj)
{
	...
	case OBJ_LINE:
		if (obj.type2 >= LINE_NORMAL_WIRE0 && obj.type2 <= LINE_NORMAL_WIRE3)
	...
	case OBJ_POINT:
		...
		if (obj.type2 >= POINT_NORMAL_VIA0 && obj.type2 <= POINT_NORMAL_VIA3)
